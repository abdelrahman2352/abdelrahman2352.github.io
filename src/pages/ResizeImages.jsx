...
export default function ResizeImages() {
  ...
  return (
    <Layout>
      ...
      {status === 'done' && <div className="bg-green-50 border border-green-200 text-green-700 rounded-lg p-4 mb-4 flex items-center gap-2"><CheckCircle2 size={16} /> تم تغيير الحجم بنجاح!</div>}
      ...
      <button onClick={handleResize} disabled={!files.length || status === 'processing'}
        ...
        {status === 'processing' ? <span className="flex items-center justify-center gap-2"><Loader2 size={18} className="animate-spin-icon" /> جارٍ المعالجة...</span> : <span className="flex items-center justify-center gap-2"><Maximize2 size={18} /> تغيير الحجم</span>}
      </button>
